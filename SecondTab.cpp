...
CSecondTab::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd)
{
    ...
    m_pNetwork = new CHammingNetwork<double>(m_sizeX * m_sizeY, 10);
    ...
    m_Picture = new CArrayPicture(this, m_sizeX, m_sizeY);

    m_Picture->SetGranularity(1);
    ...
    LoadTrainingData(CString(_T(".\\data2\\")));
...
CSecondTab::AdjustLayout()
{
    ...
    ULONG pictureSizeX = (m_granularity + 1) * m_sizeX + 2;
    ULONG pictureSizeY = (m_granularity + 1) * m_sizeY + 2;


    m_Picture->SetWindowPos(this, workRect.left, workRect.top, pictureSizeX, pictureSizeY, SWP_NOZORDER);

    workRect.top += (pictureSizeY + 3);
    workRect.bottom = workRect.top + 30;
    workRect.left += (pictureSizeX / 2  - 50);
    ...
    workRect.bottom = workRect.top + 30;
    workRect.left = pictureSizeX + 265;
    workRect.right = workRect.left + 100;
    ...
    workRect.left = pictureSizeX + 250;
...
CSecondTab::UpdateState()
{
    ...
    for (INT_PTR i = 0; i < curPicture.GetCount(); i++)
    {
        ...
        ULONG grayScale = (30 * (ULONG)GetRValue(color) + 59 * (ULONG)GetGValue(color) + 11 * (ULONG)GetBValue(color)) / 100;
    ...
    std::vector<double> result = m_pNetwork->forward(learnArray);
    ...
    double maxPossible = static_cast<double>(0.5 * m_sizeX * m_sizeY);

    std::vector<double> confidence(result.size());

    for (int i = 0; i < result.size(); ++i)
        confidence[i] = result[i] / maxPossible;


    for (INT_PTR i = 0; i < m_classes.GetCount(); i++)
    {
        m_classes[i].m_InfoWindow.SetNumber((float)(confidence[i] * 100.0f));
