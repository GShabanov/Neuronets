...
BOOL
CThirdTab::LoadImageFile(const CString &filename)
{
    IPicture* pPicture;
    IStream*  pStream;
    HRESULT   hr;
...
BOOL
CThirdTab::LoadTrainingData(const CString &path)
{
    CString  searchPath;
...
void
CThirdTab::OnSelectPicture(CListCtrlMy::DRAW_CONTEXT* pContext)
{
...
BOOL
CThirdTab::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
...
void
CThirdTab::ControlCallback(CWnd* from, void* data)
{
    if (from != m_Picture)
        return;
...
void
CThirdTab::NetworkUpdateCallback(const CArray<double>& output)
{
